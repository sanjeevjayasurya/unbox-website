...
const Blogs = ({ initialFeaturedBlog = null, initialBlogsPage = null }) => {
  const { featuredBlog, isLoading: featuredLoading } =
    useFeaturedBlog(initialFeaturedBlog);
  const { blogs, isLoading, size, setSize, hasMore, isValidating } = useBlogs(
    3,
    {
      initialPage: initialBlogsPage,
      excludeSlug: featuredBlog?.slug || initialFeaturedBlog?.slug,
    },
  );
  ...
  return (
    <>
      ...
      <div className="resources-main-div-2">
        <div className="resources-title-main-div">
          <h1 className="font-40-regular color-white">Blogs</h1>
      ...
      {(isInitialLoading || blogs?.length > 0) && (
        <div className="bg-[#FDFDFD] class-gap">
          <div className="flex flex-row gap-4 items-center justify-start max-[1024px]:justify-center">
            <p className="font-40-regular text-[#141313] ">Blogs</p>
          </div>
